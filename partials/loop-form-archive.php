		...
		<div class="row">
			<div class="col-md-12">
				<h2 class="no_bar"><?php printf( __( '%s', 'skematik' ), '<span>' . single_cat_title( '', false ) . '</span>' ); ?> Forms</h2>
			</div>
		</div>
		<div id="quick_search_sidebar">
            ...
            $cat_id = $category->term_id;
				$parents = get_ancestors( $cat_id, 'formcategory' );
				foreach ($parents as $parent) {
					$key = get_term($parent, 'formcategory');
					$parent = $key->term_id;
				}
            //echo $parent;
			?>
			<ul class="square_list">
				<?php 
					...
					foreach ($categories as $cat) { ?>
						<li <?php if ($cat->term_id == $cat_id) { ?> class="active"<?php } ?>><a href="<?php bloginfo('url'); ?>/formcategory/<?php echo $cat->slug; ?>" title="<?php echo $cat->name; ?>"><?php echo $cat->name; ?></a></li>
				<?php } ?>
			</ul>
		</div>
		<div class="clear"></div>
